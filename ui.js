// ═══════════════════════════════════════════
// UI — settings panel and monitoring
// ═══════════════════════════════════════════
...
export function syncUI() {
    ...
    if (monitorBlock && monitorContent) {
        if (p.isPregnant && (p.pregnancyWeeks > 0 || p.conceptionDate)) {
            ...
            let riskFactors = [];
            if (p.fetusCount >= 2) riskFactors.push('Multiple pregnancy');
            if (weeks > duration) riskFactors.push('Overdue pregnancy');
            if (p.complications.length > 2) riskFactors.push('Multiple complications');

            const riskHTML = riskFactors.length > 0 
                ? `<div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-solid fa-triangle-exclamation"></i> Risks:</span><span class="pregnancy-info-value" style="color: #ffaa00; font-size: 11px;">${riskFactors.join(', ')}</span></div>`
                : '';
            ...
            if (p.complications && p.complications.length > 0) {
                ...
                complicationsHTML = `<div class="pregnancy-complications"><div class="pregnancy-complications-title"><i class="fa-solid fa-clipboard-list"></i> Complications:</div>${recent.map(c => {
                    ...
                    return `<div class="complication-item" style="${resolvedStyle}"><span style="color: ${col};">${ico}</span> <strong>${c.type}</strong> <span style="opacity: 0.5; font-size: 10px;">(${c.week} wks.)${c.resolved ? ' — treated' : ''}</span><div style="font-size: 11px; opacity: 0.7;">${c.description}</div></div>`;
                }).join('')}`;
                
                // Translated note
                if (unresolvedCount > 0) {
                    complicationsHTML += `<button id="repro-doctor-btn" class="menu_button" style="margin-top: 10px; width: 100%; background: linear-gradient(135deg, #4dabf7 0%, #228be6 100%);"><i class='fa-solid fa-hospital'></i> Doctor visit (${unresolvedCount} comp.)</button>`;
                }
            ...
            monitorContent.innerHTML = `
                <div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-solid fa-hourglass-half"></i> Term:</span><span class="pregnancy-info-value">${weeks}/${duration} wks. (${days} days)</span></div>
                <div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-regular fa-calendar-check"></i> due date:</span><span class="pregnancy-info-value">${dueDateStr}</span></div>
                <div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-solid fa-baby"></i> Fetus:</span><span class="pregnancy-info-value">${fetusText} ${sexIcons}</span></div>
                <div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-solid fa-ruler"></i> Size:</span><span class="pregnancy-info-value" style="font-size: 11px;">${fetusSize}</span></div>
                <div class="pregnancy-info-row"><span class="pregnancy-info-label"><i class="fa-solid fa-heart-pulse"></i> Health:</span><span class="pregnancy-info-value" style="color: ${healthColor};">${healthIcon} ${healthText}</span></div>
                ${riskHTML}
                <div class="pregnancy-progress-bar"><div class="pregnancy-progress-fill" style="width: ${progressPercent}%"></div></div>
                <div style="text-align: center; font-size: 11px; opacity: 0.7; margin-bottom: 10px;">${progressPercent}% until birth</div>
                <div class="pregnancy-symptoms"><div class="pregnancy-symptoms-title"><i class="fa-solid fa-stethoscope"></i> Symptoms:</div><div class="pregnancy-symptoms-text">${symptoms}</div></div>
                <div class="pregnancy-recommendations"><div class="pregnancy-recommendations-title"><i class="fa-solid fa-lightbulb"></i> Recommendations:</div><div class="pregnancy-recommendations-text">${recommendations}</div></div>
                ${complicationsHTML}
            `;
...
export function setupUI() {
    try {
        // Remove the lightweight settings shell loaded by index.js, then build the full settings panel.
        $('#reproductive-system-settings').remove();
...
<div id="reproductive-system-settings" class="extension_container reproductive-system-settings">
    <div class="inline-drawer">
        ...
        <div class="inline-drawer-content">
            ...
            <hr>
            <div class="flex-container flexFlowColumn">
                <label><strong>Pregnancy duration</strong></label>
                <div class="flex-container" style="gap: 5px; align-items: center; margin-top: 5px;">
                    <select id="repro-duration" class="text_pole" style="width: 140px;">
                        <option value="12">12 wks. (~3 months)</option>
                        <option value="16">16 wks. (~4 months)</option>
                        <option value="20">20 wks. (~5 months)</option>
                        <option value="24">24 wks. (~6 months)</option>
                        <option value="28">28 wks. (~7 months)</option>
                        <option value="32">32 wks. (~8 months)</option>
                        <option value="36">36 wks. (~9 months)</option>
                        <option value="40">40 wks. (standard)</option>
                        <option value="custom">Custom...</option>
                    </select>
                    <input type="number" id="repro-duration-custom" class="text_pole" style="width: 60px; display: none;" min="4" max="100" placeholder="wks.">
                </div>
            </div>
            <hr>
            <div class="flex-container flexFlowColumn">
            ...
            <details id="repro-pregnancy-monitor" style="display: none; margin-top: 15px;">
                <summary style="cursor: pointer; font-weight: 600; color: #ff9ff3; padding: 8px; background: rgba(255,159,243,0.1); border-radius: 8px;"><i class="fa-solid fa-person-pregnant"></i> Pregnancy monitoring</summary>
            ...
            <div id="repro-manual-pregnancy" style="display: none; margin-top: 10px; padding: 10px; background: rgba(255,159,243,0.1); border-radius: 5px;">
                <label style="font-size: 12px; opacity: 0.8;">Manual setup:</label>
                <div class="flex-container" style="gap: 5px; margin-top: 5px; flex-wrap: wrap;">
                    ...
                    <span style="font-size: 11px; opacity: 0.7; align-self: center;">weeks of</span>
                ...
                <div class="flex-container" style="gap: 5px; margin-top: 8px; flex-wrap: wrap; align-items: center;">
                    <label style="font-size: 11px; opacity: 0.7;">RP date:</label>
                    <input id="repro-manual-rpdate" type="date" class="text_pole" style="width: 140px;">
                    <button id="repro-setpregnant" class="menu_button" style="padding: 5px 10px; background: #ff9ff3;"><i class="fa-solid fa-person-pregnant"></i> Set</button>
                </div>
            </div>
            <button id="repro-toggle-manual" class="menu_button" style="margin-top: 10px; opacity: 0.6; font-size: 11px;">Manual pregnancy</button>
...
.pregnancy-info-label i { width: 16px; text-align: center; }
        ...
        // SillyTavern normally handles inline drawers, but this fallback keeps the panel clickable if a theme/custom build misses it.
        $(document).off('click.reproDrawer', '#reproductive-system-settings .inline-drawer-toggle').on('click.reproDrawer', '#reproductive-system-settings .inline-drawer-toggle', function() {
            $(this).closest('.inline-drawer').toggleClass('is-open');
        ...
        $('#repro-setcycle').on('click', function() {
            ...
            setTimeout(() => {
                updatePromptInjection();
                syncUI();
                showNotification(`Cycle day: ${value}`, 'info');
            }, 100);
        });
        ...
        $('#repro-duration').on('change', function() {
            ...
            if (val === 'custom') {
                ...
                updatePromptInjection();
                syncUI();
                showNotification(`Pregnancy duration: ${val} weeks`, 'info');
            }
        });
        ...
        $('#repro-setpregnant').on('click', function() {
            ...
            showNotification(`🤰 Pregnancy set!\n${weeks}/${duration} wks. | ${formatFetusCount(count)} | Sex: ${formatSexIcons(p.fetusSex)}`, 'success');

            $('#repro-manual-pregnancy').slideUp(200);
        });

        $('#repro-reset').on('click', function() {
            if (confirm('Reset pregnancy?')) {
                resetPregnancy();
                showNotification('Pregnancy reset', 'info');
            }
        });
